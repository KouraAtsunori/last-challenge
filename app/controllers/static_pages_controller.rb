...
def get_roadstation_list(region)
  ...
  # cloud9のエディタでコピーペーストすると文字化けして処理できなくなるので
  # Windows端末で四国整備局の情報をソース表示で保存したものをアップロードすると良いです。
  if (region == "四国")
    url = "index_skk.html"
  end
...
#          if (trindex == 1) 
       ...
       #   binding.pry
          if (array[1] != "")
          roadstation << {systemname: array[0], name: array[1], url: (u = array[2]) != nil ? base_url.merge(u).to_s : nil, 
 ...
 def get_road_station
     @user = User.find(params[:user][:id])
     @region = Region.find(params[:area][:region_id])
     @region.area
     @road_station_list = get_roadstation_list(@region.area)
     #binding.pry
    if logged_in?
      @posting = current_user.postings.build if logged_in?
      @areas = Region.all #地域の情報を取得
